...
static esp_err_t csi_window_update(csi_data_buff_index_t *buff_index)
{
    ...
    if (spent_time >= (int32_t)(s_ctx.dec_config.csi_handle_time * 2) || buff_index->window >= s_ctx.window_ctx.handle_window) {
        if (buff_index->window < s_ctx.window_ctx.handle_window / 3) {
            s_ctx.window_ctx.window_start_seq = s_ctx.window_ctx.next_seq;
...
#else
    ESP_ERROR_CHECK(esp_wifi_config_espnow_rate(ESP_IF_WIFI_STA, config->rate));
#endif
